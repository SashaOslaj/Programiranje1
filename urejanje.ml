...
let rec ins_sort l = 
  match l with
  | [] -> []
  | hd :: tl -> insert hd (ins_sort tl)

let ins_sort2 l = List.fold_left (fun acc x -> insert x acc) [] l
...
let rec min_and_rest l = 
  ...
  | [] -> None
  | x :: xs -> 
    let rec odstrani_prvo z = function 
       | [] -> failwith "ni najdu" 
       | x :: xs -> if x = z then xs else z :: odstrani_prvo z xs in 
     let rec aux y = function 
       | [] -> y 
       | x::xs -> aux (min x y) xs 
     in 
     let z = aux x xs in 
     Some (z, odstrani_prvo z l) 
