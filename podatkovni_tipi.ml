...
type wizard = {name : string; hp : int; ability : mana; race : race; skills : skills}


(* Napiši funkcijo ki vsakemu uroku priredi primerno šolo magije. *)
let school_of_spell = function
  | Firewall | Blaze -> Fire
  | Renounce | Banish -> Necrotic
  | Resurrect | Cripple -> Angelic
...
let rec strongest_wizard (wizards : wizard list) : wizard option =
  ...
  | [] -> None
  | w :: ws -> 
    let stronest_ws = strongest_wizard ws in
	  (match stronest_ws with 
	   | None -> Some w 
	   | Some v -> if w.ability > v.ability then Some w else Some v)
...
let damage_caused spell target =
  int_of_float (float_of_int (mana_of_spell spell) *. (coeficient (vulnerable spell target)))
