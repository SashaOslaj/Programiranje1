...
let rec follow directions t = 
  ...
  | (Empty, _) -> None
  | (Node(l, _, _), (Left :: directs)) -> follow directs l 
  | (Node(_, _, r), (Right :: directs)) -> follow directs r
  | (Node(_, x, _), []) -> Some x
...
let rec prune directions t = 
  match (t, directions) with
  | (Empty, _) -> Empty
  | (Node(l, x, r), (Right :: directs)) -> (Node((prune directs l), x, r))
  | (Node(l, x, r), (Left :: directs)) -> (Node(l, x,(prune directs r)))
  | (_, []) -> Empty
...
let rec map_tree f t = 
  ...
  | Node(l, x, r) -> 
     if (f x) 
	 then Node((map_tree f l), true, (map_tree f r))
	 else Node((map_tree f l), false, (map_tree f r))
...
let tree_sort l = 
  ...
  | x :: y :: tl ->
    if x < y 
	then [x] @ [y] @ (tree_sort tl)
	else [y] @ [x] @ (tree_sort tl)
